...
class SAC_Agent:
    def __init__(self, state_dim, hidden_dim, action_dim, gamma, tau, alpha, buffer_size, batch_size):
        ...
        self.Qnet1_optimizer = optim.Adam(self.Qnet1.parameters(), lr=1e-4)
        self.Qnet2_optimizer = optim.Adam(self.Qnet2.parameters(), lr=1e-4)

        continuous_dim = action_dim - 2
        discrete_dim = 2

        self.target_alpha = -(0.75 * continuous_dim + 0.25 * discrete_dim)
        self.log_alpha = torch.tensor(np.log(alpha), dtype=torch.float32, requires_grad=True)
        self.log_alpha_optimizer = optim.Adam([self.log_alpha], lr=5e-5)
